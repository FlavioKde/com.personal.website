...
const GitHubStats = () => {
  ...
  return (
    <div className={cn('h-full flex flex-col', vscodeStyles.bg.tertiary, 'rounded border border-[#454545] p-4')}>
      {/* Vertical statistics - VSCode style */}
      ...
      <div className="flex flex-col gap-2 h-full">
        <div className={cn('flex items-center justify-between p-2 rounded border border-[#454545]', vscodeStyles.bg.secondary)}>
          ...
          <div className={cn('text-lg font-bold', vscodeStyles.ui.blue)}>{stats.publicRepos}</div>
        </div>

        <div className={cn('flex items-center justify-between p-2 rounded border border-[#454545]', vscodeStyles.bg.secondary)}>
          <div className={cn('text-xs', vscodeStyles.text.muted)}>Seguidores</div>
          <div className={cn('text-lg font-bold', vscodeStyles.ui.green)}>{stats.followers}</div>
        </div>

        <div className={cn('flex items-center justify-between p-2 rounded border border-[#454545]', vscodeStyles.bg.secondary)}>
          <div className={cn('text-xs', vscodeStyles.text.muted)}>Siguiendo</div>
          <div className={cn('text-lg font-bold', vscodeStyles.ui.purple)}>{stats.following}</div>
        </div>

        <div className={cn('flex items-center justify-between p-2 rounded border border-[#454545]', vscodeStyles.bg.secondary)}>
          <div className={cn('text-xs', vscodeStyles.text.muted)}>Años en GitHub</div>
          <div className={cn('text-lg font-bold', vscodeStyles.ui.orange)}>{yearsOnGitHub}</div>
        </div>
      </div>
    </div>
  );
